			...
			<div class="col-lg-8">
				...
				<div class="card shadow-sm mb-4">
					<div class="card-body">
						...
						<hr>
						<div class="d-flex">
							<button class="btn btn-outline-secondary btn-sm ms-auto" data-bs-toggle="modal" data-bs-target="#editAddressModal">변경</button>
						</div>
					</div>
				</div>
							...
							<div class="d-flex align-items-center mt-3">
								...
								<div>
									...
									<p class="text-muted mb-1">
										지금 결제 시 <span class="fw-bold">내일 도착보장</span>
									</p>
								</div>
							</div>
							<div class="mt-3">
								<p class="mb-1">
									<span class="fw-bold">수량:</span> ${item.order_quantity}개
								</p>
								<p class="fw-bold text-end">
									<fmt:formatNumber value="${item.product_price}" type="number" groupingUsed="true" />
									원 <br> <b><fmt:formatNumber value="${item.total_price}" type="number" groupingUsed="true" />원</b>
								</p>
							</div>
							<hr>
						</c:forEach>
						<div class="d-flex justify-content-between">
							<span class="fw-bold">총 주문금액</span> <span class="fw-bold" style="color: #ff6600"> <fmt:formatNumber value="${totalCost}" type="number" groupingUsed="true" />원
							</span>
						</div>
					</div>
				</div>
				...
				<div class="card shadow-sm" style="margin-bottom: 20px;">
					<div class="card-body">
						...
						<div class="form-check">
							<input class="form-check-input" type="radio" name="paymentMethod" id="cardPayment" checked> <label class="form-check-label" for="cardPayment">카드 결제</label>
						</div>
					</div>
				</div>
			</div>
			...
			<div class="col-lg-4">
				<div class="card shadow-sm">
					<div class="card-body">
						<div class="mb-3">
							<div class="d-flex justify-content-between">
								<span class="price-detail">상품 금액</span> <span> <b> <fmt:formatNumber value="${totalCost}" type="number" groupingUsed="true" />원
								</b>
								</span>
							</div>
							<div class="d-flex justify-content-between">
								<span class="price-detail">배송비</span> <span>0원</span>
							</div>
							<div class="d-flex justify-content-between">
								<span class="price-detail">상품 할인</span> <span>- 0원</span>
							</div>
							<div class="d-flex justify-content-between">
								<span class="price-detail">쿠폰 할인</span> <span>0원</span>
							</div>
							<div class="d-flex justify-content-between">
								<span class="price-detail">포인트 사용</span> <span>0원</span>
							</div>
							<hr>
							<div class="d-flex justify-content-between">
								<span class="final-price"><b>최종 결제 금액</b></span> <span class="final-price" style="font-size: 20px;"> <b> <fmt:formatNumber value="${totalCost}" type="number" groupingUsed="true" />원
								</b>
								</span>
							</div>
							<div class="d-flex justify-content-between mt-2">
								<span class="price-detail">적립 예정 포인트</span> <span>0p</span>
							</div>
						</div>
						<form action="/processPayment" method="post">
							<input type="hidden" id="deliveryContact" name="name" value="${paymentMember.name}"> <input type="hidden" id="deliveryContact" name="phone_num" value="${paymentMember.phone_num}"> <input type="hidden" id="zipcode" name="zipcode" value="${paymentMember.zipcode}"> <input type="hidden" id="deliveryAddress" name="address" value="${paymentMember.address}"> <input type="hidden" id="deliveryAddress" name="detailaddress" value="${paymentMember.detailaddress}">
			...
			<div class="modal-content">
				...
				<div class="modal-body">
					<form id="editAddressForm">
						<div class="mb-3">
							<label for="recipient" class="form-label">수령인</label> <input type="text" class="form-control" id="recipient" name="recipient" value="${paymentMember.name}">
						</div>
						<div class="mb-3">
							<label for="phone" class="form-label">전화번호</label> <input type="text" class="form-control" id="phone" name="phone" value="${paymentMember.phone_num}">
						</div>
						<div class="mb-3">
							<label for="zipCode" class="form-label">우편번호</label> <input type="text" class="form-control" id="zipCode" name="zipCode" value="${paymentMember.zipcode}" readonly>
						</div>
						<div class="mb-3">
						...
						<div id="addressDetail" class="mb-3">
							<label for="addressDetail" class="form-label">상세 주소</label> <input type="text" class="form-control" id="detailAddress" name="detailAddress" value="${paymentMember.detailaddress}">
						</div>
					</form>
				</div>
