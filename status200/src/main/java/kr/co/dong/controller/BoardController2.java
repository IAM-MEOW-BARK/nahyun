...
public class BoardController2 {
	...
	public ModelAndView list() {
		...
		mav.addObject("list", list); // 값 담아가유 (M)
		mav.setViewName("list"); // 화면 찾아가유 (V)
		return mav;
	}
	...
	public String register(BoardDTO boardDTO, HttpServletRequest request,RedirectAttributes rttr) throws Exception {
		...
		return "redirect:list"; // redirect는 페이지 이동(페이지 바뀌어요, 값 뭐 안 가져가요)
	}	
	
	@RequestMapping(value = "board/detail", method = RequestMethod.GET)
	/* 주석 처리 되어 있었음 */
	/*
	 * public ModelAndView detail(@RequestParam("bno")int bno) { ModelAndView mav =
	 * new ModelAndView(); //비지니스모델 , 서비스 BoardDTO boardDTO =
	 * boardService.getDetail(bno); boardService.updateReadCnt(bno);
	 * 
	...
	/*여기 주석 처리 되어 있었음replyDelete 수정 전(replyDelete2) */
	/*
	 * public int replyDelete(@RequestParam("reno")int reno) { return
